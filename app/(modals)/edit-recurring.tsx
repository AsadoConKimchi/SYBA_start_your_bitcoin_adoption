...
export default function EditRecurringScreen() {
  ...
  const { recurrings, updateRecurring } = useRecurringStore();
  ...
  const recurring = recurrings.find(r => r.id === recurringId);
  ...
  useEffect(() => {
    if (recurring) {
      ...
      setAmount(recurring.amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','));
      setFrequency(recurring.frequency);
      setDayOfMonth(recurring.dayOfMonth);
      setMonthOfYear(recurring.monthOfYear ?? 1);
      setPaymentMethod(recurring.paymentMethod as PaymentMethod);
      setSelectedCardId(recurring.cardId ?? null);
      setLinkedAssetId(recurring.linkedAssetId ?? null);
      setStartDate(new Date(recurring.startDate));
      setMemo(recurring.memo ?? '');

      // Category
      const isKnown = activeExpenseCategories.some(c => c.name === recurring.category);
      if (isKnown) {
  ...
  const handleSave = async () => {
    ...
    try {
      await updateRecurring(recurringId, {
        ...
        linkedAssetId: paymentMethod === 'bank' ? linkedAssetId ?? undefined : undefined,
        startDate: formatDateString(startDate),
        memo: memo || undefined,
      }, encryptionKey);
      Alert.alert('', t('recurring.editDone'), [
    ...
    <SafeAreaView edges={['bottom', 'left', 'right']} style={{ flex: 1, backgroundColor: theme.background }}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 20, borderBottomWidth: 1, borderBottomColor: theme.border }}>
          <Text style={{ fontSize: 18, fontWeight: 'bold', color: theme.text }}>{t('recurring.editTitle')}</Text>
        ...
        <ScrollView style={{ flex: 1, padding: 20 }} contentContainerStyle={{ paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
          ...
          <View style={{ marginBottom: 24 }}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {activeExpenseCategories.map(cat => (
                ...
                >
                  <Text style={{ fontSize: 14, color: category === cat.name && !showCustomCategory ? '#FFFFFF' : theme.textSecondary }}>{cat.icon} {t('categories.' + cat.id)}</Text>
                </TouchableOpacity>
              ))}
              <TouchableOpacity
          ...
          <View style={{ marginBottom: 24 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              {(['monthly', 'yearly'] as const).map(f => (
                ...
                >
                  <Text style={{ fontSize: 14, color: frequency === f ? '#FFFFFF' : theme.textSecondary }}>{t(`recurring.${f}`)}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
          ...
          {/* Card / Account selectors (same as add-recurring) */}

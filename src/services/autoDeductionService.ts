...
import { generateRepaymentSchedule, calculatePaidMonths } from '../utils/debtCalculator';
...
export async function processLoanRepayments(): Promise<{
  ...
  // 미래 자동 기록 정리 (이전 버전 버그로 생성된 기록 처리)
  {
    const { records, deleteRecord } = useLedgerStore.getState();
    const futureAutoRecords = records.filter(
      (r) => r.type === 'expense' && (r as Expense).isAutoGenerated
        && (r as Expense).linkedLoanId && r.date > todayStr
    );

    // 미래 기록이 있는 대출의 paidMonths 재계산
    const affectedLoanIds = new Set(
      futureAutoRecords
        .map((r) => (r as Expense).linkedLoanId)
    ...
    for (const loanId of affectedLoanIds) {
      ...
      const correctedPaidMonths = calculatePaidMonths(loan.startDate, repaymentDay);
      if (correctedPaidMonths < loan.paidMonths) {
        await updateLoan(loan.id, { paidMonths: correctedPaidMonths }, encryptionKey);
        console.log(`[AutoDeduction] paidMonths 정정: ${loan.name} ${loan.paidMonths} → ${correctedPaidMonths}`);
      }
    }

    for (const record of futureAutoRecords) {
      await deleteRecord(record.id);
      console.log(`[AutoDeduction] 미래 자동 기록 삭제: ${record.date}`);
    }
  }
    ...
    try {
      ...
      if (loan.paidMonths > 0) {
        ...
        for (let month = 1; month <= loan.paidMonths; month++) {
          ...
          // 미래 날짜 기록 생성 방지
          if (payment.date > todayStr) continue;

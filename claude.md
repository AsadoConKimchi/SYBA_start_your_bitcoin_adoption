...
**잘못된 행동:**
- EAS 무료 한도 소진 → 로컬 빌드로 전환한 상황을 기억하지 못함
- 버전 업데이트 없이 GitHub 푸시만 진행
- 로컬 빌드 대신 EAS 클라우드 빌드 트리거 시도

**문제점:**
- 불필요한 EAS 빌드 트리거 (한도 낭비 가능)
- 버전 관리 누락 (0.1.2 → 0.1.3 미적용)
- 사용자가 직접 지적할 때까지 인지 못함

**교훈:**
```
✅ 빌드 전 확인사항:
1. 현재 빌드 환경 확인 (EAS 클라우드 vs 로컬)
2. 로컬 APK 파일 존재 여부로 최근 빌드 방식 파악
3. app.json 버전 업데이트 확인
4. 이전 세션의 빌드 방식 기억/확인

[로컬 빌드 워크플로우]
1. app.json 버전 업데이트
2. cd android && ./gradlew assembleRelease
3. cp android/app/build/outputs/apk/release/app-release.apk syba-v{버전}-android.apk
4. 커밋 & 푸시 (APK는 .gitignore로 제외됨)
```

---

### 15. CI/CD 워크플로우 위치 확인 누락
